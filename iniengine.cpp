...
void IniEngine::loadExchangeLock(QString exchangeIniFileName, CurrencyPairItem& tempDefaultExchangeParams)
{
    ...
    instance->defaultExchangeParams = tempDefaultExchangeParams; //load defaul value from exchange
    qDebug() << "exchangeIniFileName" << exchangeIniFileName;
...
#if 1
/* exchange sau khi khai bao se setupAPI -> loadExchangeLock  will connect to loadExchange() */
void IniEngine::loadExchange(QString exchangeIniFileName)
{
    ...
    qDebug() <<"exchangeResourceFileName: " << exchangeResourceFileName;
...
void IniEngine::parseExchange(QString exchangeFileName)
{
    ...
    for (int n = 0; n < symbolList.count(); n++)
    {
        ...
        qDebug() << symbolList.at(n) << "\tcurrentPair.name: " << currentPair.name;
